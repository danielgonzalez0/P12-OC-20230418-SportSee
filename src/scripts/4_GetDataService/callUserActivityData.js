...
import { GetActivityDataFromDB, UserDataActivity } from '../3_factories/UserDataActivityFactory';
...
export const getUserDataActivity = async (id) => {
  try {
    if (!id) {
      ...
      const getData = await new GetActivityDataFromDB(url, id, getType);
      if (getData instanceof SpecificError === true) {
        throw getData;
      } else {
        if (getData.length > 0) {
          const userData = new UserDataActivity(getData, getType);
          return userData;
        } else {
